...
- 一斉配信のメール(配信停止リンク付き)で、件名に「面接」「締切」などの重要語(`config.toml` の `important_keywords`)が無いものは広告とみなし、Gemini に送らず `shukatsu-mail/skipped` ラベルを付けて飛ばす。Gmail でこのラベルをたまに眺めて、大事な連絡が混ざっていたら重要語を追記する
...
- 無料枠には回数制限がある(gemini-3.6-flash は1分5回・1日20回、2026-09-26 時点)。1回の実行で Gemini に送るのは最大5件、間隔は13秒以上にしている。制限に当たったらその回は打ち切り、残りは次回に回る
