...
Gmail に届いた就活メールを Claude API で読み取り、Notion の「就活スケジュール」に**未確認**の状態で追加するツール。
新しい画面は作らない。いつも通り Notion を見て、内容を確かめて「確認済」にするだけで企業ノートに反映される。

要件定義: https://claude.ai/code/artifact/f9ff4429-ca39-4979-93aa-607aeefa9c3b

## 流れ

```
GitHub Actions(30分ごと)
  └ stage: Gmail の新着就活メール → Claude で抽出 → 就活スケジュールに「未確認」で追加
...
https://console.anthropic.com で API キーを発行する。
...
- **実行間隔と無料枠**: 非公開リポジトリの無料枠は月2,000分で、1回の実行は1分として数えられる。15分ごとだと月約2,900分で超えるので、既定は30分ごと(約1,440分)にしている。公開リポジトリなら無料枠の制限がないので `sync.yml` の cron を `*/15 * * * *` にしてよい(コードに個人情報は入っていない。ただし公開リポジトリのスケジュール実行は、60日間コミットが無いと自動で止まる)
...
- Claude API の費用の目安(未実測): 既定の `claude-opus-5` で1通あたり数円〜10円程度。1日20通なら月3,000〜6,000円ほどになりうる。実測して高ければ `config.toml` の `model` を `claude-sonnet-5` などに下げられる(精度とのトレードオフ)
